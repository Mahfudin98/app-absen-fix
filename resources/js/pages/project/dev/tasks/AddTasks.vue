                ...
                <div class="list-wrapper" v-show="loading == false">
                    <ul class="d-flex flex-column-reverse text-white">
                        <li v-for="(row, index) in slug" :value="row.id" :key="index">
                            <vs-checkbox line-through :id="row.to_do" v-model="row.status" @click="update(row.id)">
                                {{ row.to_do }}
                            </vs-checkbox>
...
export default {
    ...
    methods: {
        ...mapActions('taskproject', ['viewDev', 'submitTaskProjectDev', 'deleteTaskDev', 'updateTaskDev']),
